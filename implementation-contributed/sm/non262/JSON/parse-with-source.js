...
/*---
includes: [compareArray.js, deepEqual.js, non262-JSON-shell.js, non262-shell.js, shell.js]
flags:
- noStrict
description: |
  pending
...
(function checkCrossCompartmentWrappers() {
    ...
    assert.sameValue(isCCW(gbl.context), true);
    ...
    assert.sameValue(isCCW(gbl.sourceV), true);
    ...
    assert.sameValue(gbl.eval("isCCW(v2)"), true);

    gbl.eval("objCCW = {};");
    assert.sameValue(JSON.isRawJSON(gbl.objCCW), false, "isRawJSON() should accept CCW arguments");
    rawJSONCCW = gbl.eval("JSON.rawJSON(455);");
    assert.sameValue(JSON.isRawJSON(rawJSONCCW), true, "isRawJSON() should return true for wrapped rawJSON objects");

    assert.sameValue(rawJSONCCW.rawJSON, "455", "rawJSON object enumerable property should be visible through CCW");

    objWithCCW = { ccw: rawJSONCCW, raw: JSON.rawJSON(true) };
    assert.sameValue(JSON.stringify(objWithCCW), '{"ccw":455,"raw":true}');
    assert.sameValue(isCCW(rawJSONCCW), true);
})();
